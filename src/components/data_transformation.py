...
class DataTransformation:
    ...
    def initiate_data_transformation(self) -> DataTransformationArtifact:
        ...
        try:
            ...
            # --- FIX START: Drop NaNs from the Target Column immediately ---
            logging.info("Checking for NaNs in target column...")
            if train_df[TARGET_COLUMN].isna().sum() > 0:
                logging.info(f"Found {train_df[TARGET_COLUMN].isna().sum()} NaNs in train target. Dropping them.")
                train_df = train_df.dropna(subset=[TARGET_COLUMN])

            if test_df[TARGET_COLUMN].isna().sum() > 0:
                logging.info(f"Found {test_df[TARGET_COLUMN].isna().sum()} NaNs in test target. Dropping them.")
